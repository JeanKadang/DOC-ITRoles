...
> **Last Updated:** July 2026 (Review 4) — Status update: role template extended with Reports To/Direct Reports, Role Scope & Boundaries, and Interaction Mode fields (backfill across 216 roles pending); 8 service-operations and governance roles added; 4 duplicate role titles resolved (a 4th, `Java Platform Product Owner`, was found and fixed 2026-07-06 after the original 3); web viewer maturity pass (validation tooling, tests, security headers, export, stale-role tracking, count-drift and domain-metadata checks — see CHANGELOG.md). Catalogue now 216 roles across 32 domains (2026-07-06: `cloud_cost_optimization_standards.md` reclassified as a reference doc, not a role, and a duplicate `Java Platform Product Owner` file removed — see "Technology-specific experience levels" and CHANGELOG.md for both). See status indicators below.
>
